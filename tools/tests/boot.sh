#!/usr/bin/env bash
# Boot kernel-debug-vm, run run.sh [exNN|proof:exNN...] in the guest as root,
...
$V destroy kernel-debug-vm >/dev/null 2>&1; $V undefine kernel-debug-vm >/dev/null 2>&1
export VM_NAME=kernel-debug-vm KERNEL_IMG="$KIMG" CMDLINE VM_DISK_ABS="$PWD/ft_linux/lfs.qcow2" PWD \
...
$V define "$DOMAIN" >/dev/null && $V start kernel-debug-vm || exit 1
